...
class PenulHeaderController extends Controller
{
 ...
 public function actionImportdata($id)
 {
  ...
  $modelback = $this->findModel($id);
  var_dump($modelback); die;
...
//same ending
      ...
      //  exit;
       ...
       foreach($ctkPenulDatatransaks as $ctkPenulDatatransakss){
        $spreadsheet->getActiveSheet()->setCellValue('A'.$baseRow, $baseRow-16);
        // $spreadsheet->getActiveSheet()->setCellValue('A'.$baseRows, $baseRows-39);
...
//$spreadsheet->getActiveSheet()->getStyle('G61')->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
      ...
      $spreadsheet->getActiveSheet()
      ->setCellValue('A15','NO.')
      ->setCellValue('B15','KODE KANTOR')
      ->setCellValue('C15','NOMOR')
      ->setCellValue('D15','TANGGAL')
      ->setCellValue('E15','NPWP')
      ->setCellValue('F15','PERUSAHAAN')
      ->setCellValue('G15','NO SERI BARANG')
      ->setCellValue('H15','URAIAN BARANG')
      ->setCellValue('I15','HS')
      ->setCellValue('J15','TARIF')
      ->setCellValue('K15','NILAI')
      ->setCellValue('L15','HS')
      ->setCellValue('M15','TARIF')
      ->setCellValue('N15','NILAI')
      ->setCellValue('O15','KET');

      $spreadsheet->getActiveSheet()->setCellValue('A16','1.')
      ->setCellValue('B16','2')
      ->setCellValue('C16','3')
      ->setCellValue('D16','4')
      ->setCellValue('E16','5')
      ...
      $spreadsheet->getActiveSheet()
      ->setCellValue('H11', 'KERTAS KERJA PENELITIAN ULANG')
      ->setCellValue('H12', $ctkPenulHeader ->kkp);

      $spreadsheet->getActiveSheet()
      ->setCellValue('M1', 'LAMPIRAN XIV')
      ->setCellValue('M2', 'PERATURAN DIREKTUR JENDERAL BEA DAN CUKAI')
      ->setCellValue('M3', 'NOMOR : PER 25/BC/2019')
      ->setCellValue('M4', 'TENTANG')
      ->setCellValue('M5', 'PERUBAHAN PERDIRJEN NO -08 /BC/2017')
      ->setCellValue('M6', 'PERUBAHAN PERDIRJEN NO -08 /BC/2017 ')
      ->setCellValue('M7', 'TENTANG PETUNJUK PELAKSANAAN PENELITIAN ULANG');

      $spreadsheet->getActiveSheet()
      ->setCellValue('O10', 'ref_id-'.$ctkPenulHeader ->id);
      ...
      $spreadsheet->getActiveSheet()
      ->setCellValue('G14', $ctkJendok ->name)
      ->setCellValue('F14', 'JENIS DOKUMEN')
      ->setCellValue('I14', 'DIBERITAHUKAN')
      ->setCellValue('L14', 'SEHARUSNYA');
      $spreadsheet->getActiveSheet()->setCellValue('F'.(string)($baseRows + 1), 'TOTAL');
...
//$SUMRANGE = 'G'.$baseRowG.':G'.$baseRows - 1;
  ...
  // die();
      ...
      $spreadsheet->getActiveSheet()->setCellValue('C'.(string)($baseRows + 8), 'Kepala Bidang PPC II')
      ->setCellValue('G'.(string)($baseRows + 8), 'Kepala Seksi Pabean dan Cukai Bidang PFPC II')
      ->setCellValue('J'.(string)($baseRows + 8), 'PDTA')
      ->setCellValue('M'.(string)($baseRows + 8), 'Pelaksana Pemeriksa');

      $spreadsheet->getActiveSheet()->setCellValue('C'.(string)($baseRows + 12),  $ctkKabid->name)
      ->setCellValue('G'.(string)($baseRows + 12), $ctkKasi->name)
      ->setCellValue('J'.(string)($baseRows + 12), $ctkPfpd->name)
      ->setCellValue('M'.(string)($baseRows + 12), $ctkAnalisPenyaji1->name);

      $spreadsheet->getActiveSheet()->setCellValue('C'.(string)($baseRows + 13),  $ctkKabid->nip)
      ->setCellValue('G'.(string)($baseRows + 13), $ctkKasi->nip)
      ->setCellValue('J'.(string)($baseRows + 13), $ctkPfpd->nip)
      ->setCellValue('M'.(string)($baseRows + 13), $ctkAnalisPenyaji1->nip);
      ...
      $spreadsheet->getActiveSheet()->getStyle('B15')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT)->setWrapText(true);
      $spreadsheet->getActiveSheet()->getStyle('B'.(string)($baseRow + 18))->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT)->setWrapText(true);

      $spreadsheet->getActiveSheet()->getStyle('A10:N'.(string)($baseRows + 13))->getFont()->setSize(12)
      ->setName('Arial');
      $spreadsheet->getActiveSheet()->getStyle('M1:M7')->getFont()->setSize(6)
      ->setName('Arial');
      $spreadsheet->getActiveSheet()->getStyle('A15:O16')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
      $spreadsheet->getActiveSheet()->getStyle('A'.(string)($baseRow + 18).':N'.(string)($baseRow + 19))->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
      ...
      $spreadsheet->getActiveSheet()->getStyle('A15:O'.(string)($baseRow))->applyFromArray($styleThinBlackBorderOutline);
      $spreadsheet->getActiveSheet()->getStyle('A'.(string)($baseRow+18).':N'.(string)($baseRows))->applyFromArray($styleThinBlackBorderOutline);
